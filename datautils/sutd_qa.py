...
import numpy as np
tokenizer = BertTokenizer.from_pretrained("/root/autodl-tmp/bert-base-cased") # 论文用的cased
model = BertModel.from_pretrained("/root/autodl-tmp/bert-base-cased")
...
def process_questions_oie_keybert_bert2(args):
    ...
    with open(args.annotation_file) as dataset_file:
        instances = dataset_file.readlines()
    _header = instances.pop(0)
    kw_model = KeyBERT(model='/root/autodl-tmp/all-mpnet-base-v2')
    #roberta = TransformerDocumentEmbeddings('roberta-base')
    #kw_model = KeyBERT(model=roberta)
    # Either create the vocab or load it from disk
    properties = {
